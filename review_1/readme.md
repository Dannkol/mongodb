...
En esta actividad se realizaran las siguientes consultas sobre la base de datos que creamos en la <a href="../mongodbclase1/readme.md"> clase anterior </a> revisala para continuar con esta clase.
...
- Operadores de Comparación
  ...
  ```js
  db.collection.find({ campo: { $ne: value } });
  ```

  - Es igual a

  ```js
  db.collection.find({ campo: { $eq: value } });
  ```
...
- Operadores logicos
  ...
  | Operador |                      Syntax                       |
  | :------: | :-----------------------------------------------: |
  |   $and   |   { $and: [ {expresion1}, ... ,{expresionN} ] }   |
  |   $or    |    { $or: [ {expresion}, ... , {expresion} ] }    |
  |   $not   |   { field: { $not: { operator-expression } } }    |
  |   $nor   | { $nor: [ { expresion1 }, ... ,{ expresionN } ] } |
